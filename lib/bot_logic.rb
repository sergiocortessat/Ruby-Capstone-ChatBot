  ...
  def initialize(token = nil)
    ...
    Telegram::Bot::Client.run(@token) do |bot|
      bot.listen do |message|
        ...
        when 'options'
          bot.api.send_message(chat_id: message.chat.id,
                               text: "Thanks for trying this chatbot #{message.from.first_name}. You can select some options such as Jokes (/joke), Dayly powerfull messages (/motivation), who am I (/who)")
        when 'joke'
          joke = Jokes.new
          bot.api.send_message(chat_id: message.chat.id,
                               text: "Thanks for trying some daily jokes #{message.from.first_name}. Hopefully the joke amuze you.   #{joke.response.read_body[82..-4]}")
